...
const AdminDashboard: React.FC = () => {
  ...
  const handleCreateUser = () => {
    alert(`User created: ${newUser.name} (${newUser.email})`);
    setShowCreateModal(false);
    setNewUser({ name: '', email: '', role: 'horse_owner', stableName: '' });
  };

  const stats = {
    totalUsers: mockUsers.length,
    activeUsers: mockUsers.filter(u => u.status === 'active').length,
    stableOwners: mockUsers.filter(u => u.role === 'stable_owner').length,
            ...
            <div className="glass-card luxury-border overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {mockUsers.map((user) => (
                      <tr key={user.id} className="border-b border-[#c9a962]/5 hover:bg-white/[0.02] transition-colors">
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium text-[#faf9f6]">{user.name}</p>
                        ...
                        <td className="py-4 px-6">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium border capitalize ${getStatusColor(user.status)}`}>
                            {user.status}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-sm text-[#a8a8a8]">
